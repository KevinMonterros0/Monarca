...
class _ProductListScreenState extends State<ProductListScreen> {
  ...
  Future<bool> _showExitConfirmationDialog(BuildContext context) async {
    return await showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: const Text(
                '¿Deseas salir? Todos los valores totales serán restablecidos a 0 y el carrito será limpiado.'),
            actions: <Widget>[
              TextButton(
  ...
  void _updateCart(dynamic product, int internalQuantity, double productPrice) {
  ...
  if (existingProductIndex != -1) {
    if (internalQuantity <= 0) {
      ...
      cart[existingProductIndex]['quantity'] = internalQuantity;
    }
  } else if (internalQuantity > 0) {
    cart.add({
      'id': product['id_producto'],
      'nombre': product['nombre'] == 'Agua' ? 'Pipa de agua' : product['nombre'],
      'precio': productPrice,
  ...
  void _sendPurchaseRequest(BuildContext context, String noFactura) async {
    ...
    try {
      ...
      print(purchaseResponse.statusCode);
      ...
      if (purchaseResponse.statusCode == 201) {
        ...
        for (var product in cart) {
          double productTotal = (product['precio'] as num).toDouble() *
              product['quantity'].toDouble();

          final detailResponse = await http.post(
            Uri.parse(
                'https://apiproyectomonarca.fly.dev/api/detalleCompras/crear'),
            headers: {
              'Authorization': 'Bearer $token',
              'Content-Type': 'application/json',
            },
          ...
          if (detailResponse.statusCode != 201) {
            throw Exception(
                'Error al enviar el detalle del producto ${product['nombre']}.');
          }
        }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(localContext).showSnackBar(
          const SnackBar(
              content: Text('Ocurrió un error al realizar la compra.')),
        );
      }
    }
  }
        ...
        body: Column(
          children: [
                ...
                child: widget.products.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          // Verificar si el nombre del producto es 'Agua'
                          final productName = product['nombre'] == 'Agua'
                              ? 'Pipa de agua'
                              : product['nombre'];
                          final productPrice = product['nombre'] == 'Agua'
                              ? product['precio_compra'] * 10000
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Text('Precio: Q$productPrice'),
                              trailing: Row(
                                    ...
                                    onPressed: () {
                                      if (quantities[index] > 0) {
                                        setState(() {
                                          ...
                                          final internalQuantity =
                                              product['nombre'] == 'Agua'
                                                  ? quantities[index] * 10000
                                                  : quantities[index];

                                          productTotal -= productPrice;
                                          globalTotalAmount -= productPrice;
                                          _updateCart(product, internalQuantity,
                                              productPrice);
                                        });
                                      }
                                    },
            ...
            Padding(
              ...
              child: Text('Total: Q$globalTotalAmount',
